...
public class ZipList extends AbstractZipList {
     ...
     * @return 新增结果
     */
    private boolean lPush(byte[] content) {
        return push(content, PUSH_HEAD_POS_MASK);
    }
     ...
     * @return 新增结果
     */
    private boolean rPush(byte[] content) {
        return push(content, PUSH_TAIL_POS_MASK);
    }

    /**
     * 向压缩列表的头部或尾部新增一个 entry
     *
     * @param content 新增的内容
     * @param where   新增的位置标识
     * @return 新增结果
     */
    private boolean push(byte[] content, byte where) {
        // 获取新增的位置
        int insertPos = where == PUSH_HEAD_POS_MASK ? getZipListEntryHeadPos() : getZipListEntryTailPos(zipList);
